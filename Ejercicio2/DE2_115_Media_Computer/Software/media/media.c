 ...
 *     encendido mientras se esta grabando.
 ...
 *	5. Muestra en el display HEX los tres ultimos bytes de datos recibidos
 *	   por el puerto PS/2.
 ********************************************************************************/

int main(void)
{
  	volatile int * green_LED_ptr = (int *) GREEN_LEDS_BASE;		// direccion LED verde
	volatile int * audio_ptr = (int *) AUDIO_BASE;			// direccion puerto audio
	volatile int * PS2_ptr = (int *) PS2_KEY_BASE;			// direccion puerto PS/2
	...
	/* mensaje que se visualizara en el LCD y en la MTL */
...
{
	int offset, row, col;
	int SRAM_BASE_SIN_CACHE = (SRAM_BASE + 0x080000000);  //Activando el bit mas significativo se elude la cache de datos
