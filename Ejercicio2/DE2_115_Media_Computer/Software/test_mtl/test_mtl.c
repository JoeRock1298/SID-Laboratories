...
/* Ejemplo de uso de los puertos paralelos, LCD y MTL (VGA) en DE2
 ...
 * 6. Escribe un mensaje en el centro de la pantalla MTL (VGA)
*/
...
int main(void)
{
	...
	*/
	...
	// Tener en cuenta que la memoria tiene una dimensión de 400x240
	MTL_box (19*8, 13*8, 35*8, 17*8, 0x187F); //Dibuja un cuadro azul en el centro
	//MTL_box (19*8, 13*8, 35*8, 17*8, 0x4B00); //Dibuja un cuadro morado en el centro
	...
	while(1)
	{
		...
		*(green_LED_ptr) = KEY_value;						// encender los LED verdes
		if (KEY_value != 0)									// mirar si se ha pulsado algún KEY
		{
			...
			// Wait until the key is released. otherwise the code won't continue execution
		}
...
void MTL_text(int x, int y, char * text_ptr)
{
	...
	offset = (y << 6) + x; //Adds offset so the text can start on the selected (x,y) position
	while ( *(text_ptr) )
	{
...
void MTL_box(int x1, int y1, int x2, int y2, short pixel_color)
{
	...
	for (row = y1; row <= y2; row++)
	{
		...
		while (col <= x2)
		{
			offset = (row << 9) + col; // cada fila ocupa 2^9 bits de columnas, para saltar a la fila que queremos
			//se debe desplazar el cursor ese número de filas = row* 2^9
